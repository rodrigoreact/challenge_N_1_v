...
plt.title("Ventas por Categoría T2")
plt.ylabel("Cantidad de Ventas")
plt.tight_layout()
plt.savefig("ventas_categoria_2.png")
plt.close()

# Gráfica: Top productos más vendidos
plt.figure(figsize=(8,5))
productos_mas_vendidos.plot(kind="bar", color="green")
plt.title("Top 5 Productos Más Vendidos T2")
plt.ylabel("Cantidad de Ventas")
plt.tight_layout()
plt.savefig("productos_mas_vendidos_2.png")
plt.close()

# Boxplot de calificaciones
plt.figure(figsize=(6,4))
sns.boxplot(x=df["Calificación"])
plt.title("Distribución de Calificaciones T2")
plt.savefig("calificaciones_boxplot_2.png")
plt.close()
...
if "lat" in df.columns and "lon" in df.columns:
    ...
    plt.title("Distribución Geográfica de Ventas T2")
    plt.xlabel("Longitud")
    plt.ylabel("Latitud")
    plt.savefig("mapa_geografico_2.png")
    plt.close()
...
pdf.cell(0, 10, "Informe de Análisis - Tienda 2", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
pdf.image("ventas_categoria_2.png", w=170)
pdf.image("productos_mas_vendidos_2.png", w=170)
pdf.image("calificaciones_boxplot_2.png", w=120)
if "lat" in df.columns and "lon" in df.columns:
    pdf.image("mapa_geografico_2.png", w=120)

# Guardar PDF
pdf.output("informe_tienda_2.pdf")
print("✅ Informe generado: informe_tienda_2.pdf")
